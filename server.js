...
app.post('/api/criar-parcelamento', async (req, res) => {
  try {
    ...
    // sai aqui e o pedido segue para "pendente de envio" como qualquer outro.
    // O parcelamento passa a ser um controle paralelo, não um status.
 ...
 * O cliente pagou por fora (dinheiro, Pix direto, cartão na maquininha).
 * Além de marcar como paga, o boleto correspondente é CANCELADO na Asaas:
 * se ficasse de pé, o cliente poderia pagar duas vezes a mesma parcela.
 */
app.post('/api/quitar-parcela', async (req, res) => {
  try {
    ...
    const paga = x => x.status === 'CONFIRMED' || x.status === 'RECEIVED';
    const alvos = tudo
      ? (plano.parcelas || []).filter(x => !paga(x)).map(x => x.n)
      : [Number(n)];

    if (!alvos.length) return res.status(409).json({ erro: 'Não há parcela em aberto.' });

    const parcelas = [...plano.parcelas];
    for (const num of alvos) {
      const idx = parcelas.findIndex(x => x.n === num);
      if (idx < 0) continue;
      if (paga(parcelas[idx])) continue;

      // Derruba o boleto na Asaas, se existir e ainda estiver em aberto
      if (parcelas[idx].asaasId) {
        try {
          const cob = await asaas(`/payments/${parcelas[idx].asaasId}`);
          if (cob.status === 'PENDING' || cob.status === 'AWAITING_RISK_ANALYSIS') {
            await asaas(`/payments/${parcelas[idx].asaasId}`, { method: 'DELETE' });
          }
        } catch (e) {
          console.warn(`Boleto da parcela ${num} não pôde ser cancelado:`, e.message);
        }
      }
    ...
    if (qtdPagas === plano.total) mudancas['parcelamento.quitadoEm'] = new Date().toISOString();

    await refPedido.update(mudancas);
    console.log(`Baixa manual — pedido ${pedido.codigo} — parcela(s) ${alvos.join(', ')}`);
    res.json({ ok: true, pagas: qtdPagas, total: plano.total });
  } catch (erro) {
    console.error('Falha na baixa manual:', erro);
    res.status(erro.codigo || 500).json({ erro: erro.message || 'Não foi possível dar baixa.' });
  }
});
...
async function processarParcela(refPedido, payment, status) {
  await db.runTransaction(async tx => {
    ...
    // O estoque e o status do pedido já foram tratados na aprovação: aqui só
    // se marca a parcela. O pedido segue seu caminho (separação, envio) mesmo
